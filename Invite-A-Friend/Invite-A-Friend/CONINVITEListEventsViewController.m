//
//  CONINVITEListEventsViewController.m
//  Invite-A-Friend
//
...
#import "CONINVITEListEventsViewController.h"
#import <QuartzCore/QuartzCore.h>
#import "JSON.h"
#include "CustomFriendCells.h"

@interface CONINVITEListEventsViewController ()
- (IBAction)closeView:(id)sender;
@end

@implementation CONINVITEListEventsViewController
@synthesize tweets;
@synthesize friends;
...
- (void)searchBarSearchButtonClicked:(UISearchBar *)searchBar{
    ...
    NSMutableString *searchString = [NSMutableString stringWithFormat:@"http://amber.concept96.co.uk/api/v1/pending_events/%@",@""];
...
- (void)viewDidLoad
{
    ...
    [_searchOption setScopeBarButtonTitleTextAttributes:[NSDictionary dictionaryWithObjectsAndKeys:[UIColor whiteColor], UITextAttributeTextColor, nil] forState:UIControlStateNormal];
    ...
    NSURL *searchString = [NSURL URLWithString: [NSString stringWithFormat:@"http://amber.concept96.co.uk/api/v1/events_attending/%@",@""]];
    
    NSError *error;
    
    NSDictionary *attendeesjson = [NSDictionary dictionaryWithObjectsAndKeys:  UID, @"uid", nil];
    ...
    [attendRequest setURL:searchString];
    [attendRequest setHTTPMethod:@"GET"];
    ...
    //NSURLRequest *request = [NSURLRequest requestWithURL:[NSURL URLWithString:searchString]];
    //[NSURLConnection sendSynchronousRequest:attendRequest returningResponse:&response error:&error];
